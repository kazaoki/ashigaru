...
window.addEventListener('DOMContentLoaded', function(event){
	document.querySelectorAll('[data-ckeditor]').forEach(elm => ckeditor_load(elm))
})
...
function ckeditor_load(target)
{
	...
	if ('content' === target.getAttribute('data-ckeditor')) {
		...
		let __SITE__ = document.querySelector('meta[name="ashigaru:site"]').getAttribute('content');
		let __MANAGE__ = document.querySelector('meta[name="ashigaru:manage"]').getAttribute('content');
		let __UPLOADS__ = document.querySelector('meta[name="ashigaru:uploads"]').getAttribute('content');
		...
		let cke = CKEDITOR.replace(target,
			{
				...
				// editor content css
				contentsCss: __SITE__+'/manage/assets/css/ckeditor-add.css?20231118',
				bodyClass: 'page',
		...
		cke.on('blur', function(e) {
			window.validon.send(e.editor.name)
		});

		// textareaにデータを強制同期する関数を用意（フォーカス移さずにバリデートする直前に実行など）
		target['ckeSync'] = function() {
